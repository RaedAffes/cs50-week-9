...
def buy():
    ...
    if request.method == "POST":
        ...
        if not (stock := lookup(symbol)):
            return apology("INVALID SYMBOL")

        rows = db.execute("SELECT * FROM users WHERE id = ?;", session["user_id"])

        user_owned_cash = rows[0]["cash"]
        total_prices = stock["price"] * shares
...
def sell():
    ...
    if request.method == "GET":
        ...
        if shares <= 0 or shares != int(shares):
            return apology("Shares must be valid number")
        ...
        updtd_cash = user_cash + transaction_value

        db.execute("UPDATE users SET cash = ? WHERE id = ?", updtd_cash, user_id)
